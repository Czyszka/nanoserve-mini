...
- równoległa obsługa wielu zapytań przez model zajmujący 8 kart:
  komunikacja zajmuje 83,9% czasu kroku (sekcja 6.4);
- równoległa obsługa wielu zapytań przez model zajmujący 4 karty: 53,3%
  (sekcja 6.4);
- obsługa jednego zapytania na raz przez model zajmujący 8 kart: 22,5%
  (sekcja 6.4);
- obsługa jednego zapytania na raz przez model zajmujący 4 karty: 14,8% (14,8% = 1,56 ms / 10,54 ms; sekcja 6.2);
- model zajmujący 1–2 karty, niezależnie od liczby równoległych zapytań: około 0% czasu kroku. Przejście z 1 na 2 karty wydłuża krok o 0,93 ms na 9,91 ms (przy kartach na różnych CPU tylko 0,15 ms), czyli na poziomie szumu pomiaru ±0,4 ms (sekcja 6.2). Celowe pogorszenie ścieżki, wymuszenie ruchu między kartami przez pamięć procesora (sekcja 6.3), zmienia przepustowość o mniej niż 1%.

## 2. Zakres badania i warunki testowe

Badanie wykonano na serwerze Supermicro SYS-521GE-TNRT wyposażonym w dwa
procesory Intel Xeon Gold 6530 oraz osiem kart NVIDIA H200 NVL, każda z
143 GB pamięci HBM3e. Karty komunikują się wyłącznie przez magistralę PCIe
Gen5, w topologii czterech przełączników po dwie karty, bez NVLink i bez
NVSwitch. Pełny schemat połączeń znajduje się w załączniku B. Modele językowe
są serwowane przez vLLM v0.20 uruchomiony w kontenerach Docker i udostępniane
użytkownikom przez API.

Badanie obejmuje trzy składniki procesu generowania odpowiedzi na tym serwerze.
Pierwszym jest czas pojedynczego kroku generowania, który decyduje o szybkości
odpowiedzi. Drugim jest komunikacja między kartami po PCIe, czyli jedyny
składnik, który zmieniłby zakup NVLink. Trzecim jest stały narzut silnika
serwującego, czyli czas zużywany w każdym kroku na czynności organizacyjne:
wybór zapytań do bieżącego kroku, wybór kolejnego fragmentu tekstu, obsługę
spekulacji oraz wymianę poleceń między CPU i GPU. Jeśli dominuje ten trzeci
składnik, szybsze łącze między kartami nie usunie głównego ograniczenia.
...
Punktem wyjścia badania była obserwacja, że pod pełnym obciążeniem, gdy licznik GPU-Util z popularnego narzędzia nvidia-smi wskazuje 100%, żaden zasób serwera nie zbliża się do nasycenia: pamięć HBM jest aktywna przez 7–9% czasu, jednostki obliczeniowe przez około 20%, łącze PCIe używa około 10% swojej przepustowości, a pobór mocy sięga około 30% limitu (pomiary: sekcja 6.1). Mimo to opóźnienie między generowanymi tokenami rośnie wraz z liczbą równolegle obsługiwanych zapytań, a dołożenie kolejnych kart dodatkowo je pogarsza. Standardowa diagnostyka, polegająca na poszukiwaniu nasyconego zasobu, nie wskazuje tu więc żadnej przyczyny.

Różnica między `GPU-Util` a licznikami DCGM wynika z definicji tych metryk.
`GPU-Util` z `nvidia-smi` wskazuje, czy w danym przedziale karta miała
uruchomioną pracę GPU. Nie rozróżnia jednak obliczeń, komunikacji ani oczekiwania
wewnątrz operacji. Dlatego wartość `100%` nie oznacza pełnego wykorzystania
jednostek obliczeniowych ani pamięci HBM.

Do oceny wykorzystania zasobów użyto liczników DCGM opisanych w sekcji 3,
w szczególności `SM_ACTIVE`, `DRAM_ACTIVE` oraz `PCIE_RX/TX`. Te liczniki
pokazują, że w badanych oknach aktywność jednostek obliczeniowych i pamięci HBM
była niska, a istotnym składnikiem pracy pozostawała komunikacja między kartami.
Dalsza część notatki rozdziela te składniki pomiarowo.
...
Poniżej zestawiono zbadane scenariusze obciążenia maszyny. Kolumna TP podaje,
na ile kart został podzielony model (wyjaśnienie w sekcjach 3–4), a kolumna c
(współbieżność) określa, ilu klientów serwer obsługuje jednocześnie. Wartość
c=1 odpowiada jednej rozmowie na żywo, a c=64 dziesiątkom równoległych zapytań
(np. kilka czatów na żywo plus agenci programistyczni utrzymujący po kilka
zapytań naraz). Zysk, zgodnie z prawem Amdahla, wynika z udziału komunikacji
w całkowitym czasie kroku generowania (sekcja 6.4).
...
| ~200–500 mld par. (FP8) — wymaga 4 kart | 4 | współbieżny (c=64) | kupuj | ~2,1× | 53,3% kroku |
| ~0,5–1 bln par. (FP8) — wymaga 8 kart | 8 | pojedynczy (c=1) | nie kupuj | ≤1,2–1,3× | 22,5% kroku |
| ~0,5–1 bln par. (FP8) — wymaga 8 kart | 8 | współbieżny (c≥8) | kupuj | ~2,7× (maks. 6,2×) | 83,9% kroku |

Wartości podstawy pomiarowej to udział czasu kroku dekodowania zajęty przez komunikację (all-reduce), o ile nie napisano inaczej:
...
- **53,3%, 22,5%, 83,9%** - odczyt wprost z profili czasowych kroku
  (sekcja 6.4): Qwen TP4 c=64; Kimi TP8 c=1; Kimi TP8 c=16.
...
Klasy rozmiaru modelu podano dla precyzji FP8, czyli około `1 bajt` na parametr,
ponieważ taką precyzję stosuje konfiguracja produkcyjna na badanym serwerze.

Liczba wymaganych kart zależy nie tylko od rozmiaru wag, ale także od miejsca
potrzebnego na KV cache i od maksymalnej długości kontekstu. Kimi-K2.6 ma około
`554 GB` wag. Same wagi zmieściłyby się na czterech kartach H200, po około
`138,6 GB` na kartę, ale taki podział zostawiłby zbyt mało pamięci na KV cache
i narzuty runtime'u. Dlatego w badanej konfiguracji model uruchamiano na ośmiu
kartach.
...
NCCL dobiera wariant komunikacji osobno dla każdego wywołania, na podstawie
rozmiaru komunikatu i topologii połączeń. W tym pomiarze wybrany wariant nie
został zapisany w logach, ponieważ nie włączono `NCCL_DEBUG=INFO`.

Logi startowe silnika potwierdzają natomiast, że sprzętowo przyspieszane ścieżki
scalania były nieaktywne. Własny all-reduce silnika vLLM był wyłączony, ponieważ
nie jest obsługiwany dla więcej niż dwóch GPU w konfiguracji wyłącznie PCIe
(`not supported on more than two PCIe-only GPUs`). Ścieżka z multicastem NVLink,
czyli NVLS / FlashInfer-TRT-LLM, także się nie uruchomiła
(`Device does not support multicasting ... expected on GPUs without NVSwitch`),
mimo ustawionego `NCCL_NVLS_ENABLE=1`.

Scalanie wykonywało więc standardowe NCCL, backend `nccl`, wersja `2.28.9`, po
magistrali PCIe. Dotyczy to zarówno Kimi-K2.6, jak i modelu kontrolnego Qwen.
...
1. Zbyt wolna pamięć (W_silicon) — karta czeka, aż wagi zostaną wczytane z pamięci HBM. Ślad: wysoki DRAM_ACTIVE. Badane w sekcji 6.1.
2. Zbyt kosztowna komunikacja (N_rounds × r) — czas kroku pochłaniają scalenia (all-reduce). Ślady: czas kroku rośnie wraz z liczbą kart, w profilu czasowym przeważa komunikacja, a łącze PCIe dochodzi do granicy przepustowości, mimo że rdzenie obliczeniowe pozostają bezczynne. Badane w sekcjach 6.2 (wzrost czasu kroku z liczbą kart), 6.3 (kontrola P2P) i 6.4 (udział komunikacji w profilu).
3. Zbyt duży stały narzut (F_host) — silnik serwujący obciąża każdy krok niezależnie od sprzętu. Ślady: krok pozostaje kosztowny nawet na jednej karcie, gdzie komunikacja w ogóle nie zachodzi, a w profilu przeważają przerwy, w których karta nie wykonuje żadnej operacji. Badane w sekcji 6.5, a przerwy w profilu pojedynczego klienta w sekcji 6.4.
...
w tym scenariuszu. Dalsze pomiary w sekcjach 6.2 do 6.5 rozdzielają dwa
...
Wartość 2,63 to iloraz dwóch median z pomiaru po stronie klienta, mediany czasu kroku 8,93 ms i mediany TPOT 3,39 ms. W logu silnika zmierzona długość akceptacji spekulacji wynosi 2,54. Obie liczby opisują średnią liczbę tokenów uzyskiwanych w jednym kroku, ale powstają inną drogą. Zależność czas kroku ÷ TPOT = długość akceptacji jest dokładna dla wartości średnich, natomiast tabela podaje mediany, a iloraz median nie musi być równy średniej liczonej przez silnik. Różnica około 0,09, czyli blisko 3,5%, wynika z tej metody agregacji, a nie ze sprzeczności w danych. Zgodność obu odczytów do kilku procent uwiarygodnia wynik, zamiast go podważać.
...
Zgodnie z prawem Amdahla NVLink może skrócić tylko tę część kroku, która wynika
z komunikacji między kartami GPU. Nie skraca właściwych obliczeń modelu, obsługi
spekulacji, harmonogramowania po stronie hosta ani innych stałych kosztów kroku
opisanych w sekcji 6.5. Te składniki tworzą część `1 - s` w poniższym wzorze.
Sekcja 6.5 nie dostarcza wartości `s`; pokazuje natomiast, co pozostaje w kroku
poza komunikacją. Dlatego przy pojedynczym kliencie zysk z szybszego łącza jest
ograniczony nawet wtedy, gdy samo łącze byłoby idealne.

Podejście jest zgodne z literaturą dotyczącą skalowania inferencji LLM. Praca
[*Scaling LLM Inference Beyond Amdahl’s Limits via Eliminating Non-Scalable
Overheads*](https://arxiv.org/abs/2606.01927) wykorzystuje prawo Amdahla do
opisu ograniczeń skalowania tensor parallelism i wskazuje komunikację między GPU
oraz nieskalowalne koszty runtime'u jako źródła subliniowego skalowania. Praca
[*Evaluating Modern GPU Interconnect: PCIe, NVLink, NV-SLI, NVSwitch and
GPUDirect*](https://arxiv.org/abs/1903.04611) pokazuje z kolei, że typ i
topologia połączeń GPU wpływają na efektywną komunikację między kartami. W tej
notatce udział komunikacji w czasie kroku jest mierzony lokalnie, a następnie
podstawiany do rachunku Amdahla, aby oszacować część czasu możliwą do skrócenia
przez NVLink.

Rachunek oparto na udziale komunikacji w czasie kroku. W pierwszym kroku
obliczono górną granicę dla idealnego łącza, które usuwa całą komunikację objętą
NVLink.
...
Uszczegółowiona tabela „Twierdzenie → źródło" na poziomie pliku, per sekcja.

| Sekcja | Twierdzenie (klaster) | Źródło w repozytorium |
|---|---|---|
| 6.1 | liczniki spoczynek / c=1 / c=64 (moc, `SM_ACTIVE`, `DRAM_ACTIVE`, PCIe TX/RX) | `results/runs/2026-06-10_w1_article_evidence/p0_gpu_counters/{idle,single_c1,batched_c64}_dcgmi.txt`; ukończenia w `…/bench/{single_c1,batched_c64}.json` |
| 6.2 | krzywa TP — `ITL`/`TPOT` (c=1), przepustowość i efektywność (c=64) | TP1 `results/runs/2026-06-10_extra/p0_gpu_counters/bench/{single_c1,batched_c64}.json`; TP2/4/8 `results/runs/2026-06-11_bottleneck/qwen_tp_curve/bench_tp{2,4,8}/tp{2,4,8}_c{1,64}.json`; liczniki `…/qwen_tp{2,4,8}_c64_dcgmi.txt` |
| 6.3 | kontrola UPI (TP2 `0,1` vs `0,4`; TP4 `0–3` vs `0,1,4,5`), kontrola P2P (`nop2p`) | `results/runs/2026-06-11_bottleneck/qwen_tp_curve/{bench_tp2,bench_tp2x04,bench_tp2_nop2p,bench_tp4}/`; cross-island `results/runs/2026-06-11_nvlink_boundary/qwen/bench_tp4x0145/` |
| 6.3 | zmienność TP2 `±0,4 ms` | pochodna z powtórzeń (`bench_tp2`, `bench_tp2_nop2p`, `bench_tp2x04`), bez pojedynczego pliku — nota katalogu `results/runs/2026-06-11_bottleneck/qwen_tp_curve/` |
| 6.4 | ramp Kimi c=1…32 (klient + DCGM) oraz profile czasowe | bench `results/runs/2026-06-11_nvlink_boundary/kimi_ramp/bench/kimi_c{1,8,16,16r,32}.json`; DCGM `…/kimi_ramp/kimi_c{1,8,16,16r,32}_dcgmi.txt`; profile `results/runs/2026-06-11_bottleneck/kimi_profiler/trace_summary_rank0.txt`, `results/runs/2026-06-11_nvlink_boundary/kimi_ramp/trace_summary_rank0.txt`, `results/runs/2026-06-11_nvlink_boundary/qwen/trace_summary_tp4_c64_rank0.txt` |
| 6.5 | interwencje stałego narzutu (`base`/`nospec`/`eager`/`perfgov`) | `results/runs/2026-06-11_nvlink_boundary/floor/bench/{base,nospec,eager,perfgov}_c1.json`; `SM_ACTIVE` eager `…/floor/qwen_eager_c1_dcgmi.txt` |
| 7 | wejścia `s`, `capture` | wewnętrzne odsyłacze do §6.2 / 6.3 / 6.4, wszystkie zweryfikowane co do cyfry |
| 7 | pasma `B_pcie 128` / `B_nvlink 900 GB/s` (nominalne) | [karta katalogowa NVIDIA H200 NVL](https://www.pny.com/file%20library/company/support/linecards/data-center-gpus/h200-nvl-datasheet.pdf), zob. §2; topologia `docs/operations/sys-521ge-tnrt.md`, `docs/operations/infrastructure.md` |
