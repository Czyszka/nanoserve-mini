...
2. **Wykresy Grafana pod obciążeniem (#34 + prezentacja):** schodkowy ramp
   c1→c8→c16→c32→**c64** na produkcyjnym Kimi. c64 > `max-num-seqs 32`
   celowo — dopiero wtedy zapala się panel kolejki (`num_requests_waiting`).
   Screenshoty dashboardu = domknięcie pozycji „screenshot pod obciążeniem"
   z #34 i materiał W6 do prezentacji.
...
| Cz. 4 | restore plain Kimi + stack → **ramp Grafana** c1→c8→c16→c32→c64 + screenshoty | 35 |
| Cz. 5 | snapshoty końcowe, commit | 10 |
| | **razem** | **108** |

**Kolejność cięcia:** Cz. 4 schodki c8/c16 (zostaw c1→c32→c64) → Cz. 3 okno
dcgmi (sam zrzut zostaje) → Cz. 1 notatki.
...
**W przeglądarce (przygotuj PRZED rampem):**

1. `http://<ip-serwera>:3001` → dashboard **vLLM Phase 1** (provisioned).
2. Zakres czasu: **Last 30 minutes**; auto-refresh: **10s**.
3. Zostaw otwarte — schodki pojawią się na żywo.

### 4b. Ramp schodkowy (ładne wykresy robią SIĘ SAME z tej sekwencji)

Każdy schodek to inny poziom `running`; c64 > `max-num-seqs 32` → kolejka.
Między schodkami 60 s przerwy — na wykresach powstają wyraźne stopnie.

```bash
ensure_dataset || echo "PRZERWIJ"
bench_prereqs "$COMPOSE"

kimi_step () {  # $1=c $2=num_prompts
  docker compose -f "$COMPOSE" exec vllm bash -c '
    export HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1
    vllm bench serve --backend vllm --base-url http://127.0.0.1:8000 \
      --model kimi-k2.6 --trust-remote-code --tokenizer moonshotai/Kimi-K2.6 \
      ...
      --save-result --result-dir /tmp/xbench --result-filename ramp_c'"$1"'.json'
  sleep 60
}

date +%s > "$GOUT/ramp_start_epoch.txt"
kimi_step 1  8      # wygrzewka + najnizszy schodek w jednym
kimi_step 8  64
kimi_step 16 128
kimi_step 32 256
kimi_step 64 384    # > max-num-seqs 32 -> panel WAITING sie zapala
...
### 4c. Screenshoty (w trakcie i po)

- **W TRAKCIE schodka c64:** screenshot dashboardu — panele `running` (płasko
  na 32), **`waiting` > 0** (kolejka!), KV-cache usage wysoko, throughput.
  To jest brakujący screenshot z #34.
- **PO rampie:** ustaw zakres czasu tak, by objąć cały ramp
  (epochy w `$GOUT/ramp_*_epoch.txt`) → screenshot całych schodków
  (running/throughput/ITL/TTFT/spec-decode acceptance).
- Zapis: PNG lokalnie → commit do `$GOUT/` (np. `dashboard_c64_queue.png`,
  `dashboard_full_ramp.png`) oraz najlepsze ujęcie do
  `docs/presentations/assets/` pod slajd W6.
- Opcja bez przeglądarki (kontener `grafana-image-renderer` stoi): panele da
  się wyrenderować przez `/render/d-solo/<uid>/...?panelId=N&from=<ms>&to=<ms>
  &width=1600&height=900` z autoryzacją Grafany — ale przy dostępnej
  przeglądarce screenshot jest szybszy i ładniejszy (pełny dashboard).
