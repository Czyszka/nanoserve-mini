...
- **Script or harness:** `benchmarks/scripts/request_once.py` for smoke validation, `benchmarks/scripts/measure_ttft_once.py` for one streaming TTFT/E2E record, `benchmarks/scripts/run_sequential_benchmark.py` for repeated sequential TTFT/E2E p50/p95, future vLLM `/metrics` scrape for scheduler/KV cache, future workload/cache experiment for prompt-prefix reuse.
